...
| **Per-region climate.md dossiers + national climate-projection sources (CMIP6 SSP2-4.5, IPMA, AEMET, Meteo-France DRIAS, Met Office UKCP18, NCA5, Ouranos, WICCI)** | climate_buffering | 5 | 4 | **20** | 20/20 jurisdictions; primary features + altitude + strength + trajectory; all data_confidence=high | 2020-2025 climate-projection vintages | per-jurisdiction (point at centroid) | research-dossier internal; underlying climate-projection sources typically open | **[verified by use]** — compiled via `scripts/compile_per_jurisdiction.py climate-buffering`. Delivers Overview decision 8.1's explicit "climate-buffering features (altitude, distance-to-coast, mountain-shelter exposure)" gap. |
...
Top by priority score **and** verified gettability: **legal_ownership (25)**, **water_source_control (20)**, **land_cost (20)**, **climate_buffering (20)**, Aqueduct (water ×2), UCDP (conflict), GFW (forest), OSM/GEN (regen network), **hospital_proximity (16)**, **demographic_trajectory (16)**, **soil_contamination (12 — thin by design)**. These **twelve** are the V1 processed set. Solar/soil-organic-carbon/population are high-relevance but blocked on the raster wall — V2 or an alternate fetch path. **All 6 red-line underlying datasets ingested this round** (active-conflict, water-deficit, collective-ownership, hospital-60-min-proxy, soil-contamination, single-entity-water-control). **Overview decision 8.1's climate-buffering gap is also closed.**
